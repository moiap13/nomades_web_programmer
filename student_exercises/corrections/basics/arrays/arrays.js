...
function min(tableau) {
  ...
  return tableau.reduce((pv, cv) => cv < pv ? cv : pv, Infinity)
}
...
function position(tableau, valeur) {
  // for(let i=0; i<tableau.length; i++){
  //   const currentValue = tableau[i]
  //   if(currentValue === valeur){
  //     return i
  //   }
  // }
  // return -1
...
function estTableau(tableau) {
  ...
  // return (tableau.constructor === Array);
...
function estTableauDeNombres(tableau) {
  if(!estTableau(tableau) || tableau.length === 0) return false
